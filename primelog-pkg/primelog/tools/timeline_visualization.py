...
def run(log_file: str, mode: str = "all", interval: str = "1m",
        width: int = 80, height: int = 20, top: int = 5,
        detect_anomaly: bool = False, anomaly_threshold: float = 3.0) -> None:
    ...
    if not timestamps or not events:
        ...
        return
    dt_list = [datetime.fromisoformat(ts) for ts in timestamps]
    ivl = parse_interval(interval)
    print(f"\n📁 文件: {log_file}")
    print(f"⏰ 时间范围: {min(dt_list).strftime('%Y-%m-%d %H:%M:%S')} → {max(dt_list).strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"📊 总事件数: {len(events)}")
    if detect_anomaly:
        detect_anomalies(dt_list, ivl, anomaly_threshold)
    if mode in ['heatmap', 'all']:
        print_heatmap(dt_list, ivl, width, height)
    if mode in ['wave', 'all']:
        print_wave_chart(dt_list, ivl, width, 15)
    if mode in ['timeline', 'all']:
        print_multi_timeline(events, prime_map, ivl, width, top, timestamps)
