...
def cmd_loadmark(args):
    ...
    action = "消除" if remove else "添加"
    scope  = f"递归深度={max_depth}" if args.L is not None else ("递归" if args.r else "当前目录")
...
示例:
  ...
  primelog export --out ./logs      导出当前运行日志
    ...
    # loadmark
    p_lm = sub.add_parser('loadmark', help='管理 __loadmark__ 标记文件')
    p_lm.add_argument('directory',        help='目标目录')
    p_lm.add_argument('-r', action='store_true', default=False,
                      help='递归处理所有子目录（无深度限制）')
    p_lm.add_argument('-L', type=int, metavar='深度', default=None,
                      help='递归处理子目录，限制最大深度（如 -L 3）')
    p_lm.add_argument('-x', action='store_true', default=False,
                      help='消除标记（默认是添加）')
